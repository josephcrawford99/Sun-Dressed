...
### BentoBoxCard
Inside card container. Organizes bento boxes into grid. Dresses are two blocks tall all others are one tall. Should be populating based on outfit suggestion tool. When an element is clicked it should grow to fill the container as gray rectangle with round edges.

### BentoBoxElement
A block inside the box container. Holds a logo for the individual part of an outfit. Not a photo, probably a vector image black with no background or png. Gray background square with no outlines, rounded edges. When clicked, grows to size of card and shows info about clothing item (suggested adjusted user temperature for item, good for rain? Good for wind? Thumb up and down for liking it and thumb down can be exploded into (too hot/cold or I don't want to wear this). When box "left chevron" in upper left is clicked the element shrinks back into the shape so whole outfit is visible.
