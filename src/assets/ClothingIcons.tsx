...
type IconComponent = ((props?: IconProps) => React.JSX.Element) | (() => string);
...
Object.keys(ClothingIcons).forEach(key => {
  const originalIcon = ClothingIcons[key] as IconComponent;
  if (typeof originalIcon === 'function' && originalIcon.length > 0) {
    ...
    ClothingIcons[key] = (props: IconProps = {}) => 
      (originalIcon as (props: IconProps) => React.JSX.Element)({ ...defaultProps, ...props });
  }
  // String functions remain unchanged
});

// Helper function to get icon safely with fallback
export const getClothingIcon = (iconKey: string, props: IconProps = {}): React.JSX.Element | string => {
  const IconComponent: IconComponent = ClothingIcons[iconKey] || ClothingIcons.unknown;
  ...
  return (IconComponent as (props?: IconProps) => React.JSX.Element)(props);
};